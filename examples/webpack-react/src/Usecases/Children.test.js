...
describe("Children", function() {
  const Component = ({ locale, ...props }: { locale: string }) => {
  ...
  const getText = (element, props = {}) => {
    return mount(<Component {...props} locale="cs" />)
      .find(element)
      .first()
      .text()
  }
  ...
  it("should render without provider", function() {
    expect(mount(<Children />).html()).toMatchSnapshot()
  })

  it("should render", function() {
    expect(mount(<Component locale="cs" />)).toMatchSnapshot()
  })
