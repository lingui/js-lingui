...
It is also possible to merge the translated catalogs into a single file per locale by specifying :conf:`mergePath`. For example if :conf:`mergePath` is assigned  ``locales/{locale}`` then catalogs will be compiled to ``/locales/cs.js`` and ``/locales/en.js``.
