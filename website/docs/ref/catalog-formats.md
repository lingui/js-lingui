...
| `foldLength`                  | number                          | `80`    | Maximum line width before folding long strings. When a string exceeds this length, it will be split across multiple lines. Set to `0` to disable folding (strings will only break on actual newlines)                                                                            |
| `compactMultiline`            | boolean                         | `true`  | When `true`, multiline strings start with content on the first line. When `false`, uses GNU gettext's traditional format with an empty first line. The compact format is recommended as it's compatible with translation platforms like Crowdin that may strip empty first lines |
...
| `foldLength`           | number  | `80`           | Maximum line width before folding long strings. When a string exceeds this length, it will be split across multiple lines. Set to `0` to disable folding                 |
| `compactMultiline`     | boolean | `true`         | When `true`, multiline strings start with content on the first line. When `false`, uses GNU gettext's traditional format with an empty first line                        |
