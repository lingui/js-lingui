...
The `defineMessage` (alias: `msg`) macro allows to define a message for later use. It has the same signature as `t` and returns a `MessageDescriptor` that you can pass to `i18n.t` to get a translated string at any time later. This is useful for [Lazy Translations](/guides/lazy-translations).
...
It works by taking an object with a single property, where the key becomes the placeholder name and the value is the expression:

```js
import { t, ph } from "@lingui/core/macro";
const message = t`Hello ${ph({ name: getUserName() })}`;
...
const message = i18n._(
  /** i18n */ {
    id: "OUzSsf",
    message: "Hello {name}",
    values: { name: getUserName() },
  }
);
```

The `ph` macro can be used with any other macro that accepts expressions, including [`plural`](#plural), [`select`](#select), and JSX macros like [`Trans`](#trans):
...
<Trans id={"Z5GF+g"} message="Welcome back, {username}!" values={{ username: getUser().name }} />;
```
...
The directive is placed in a comment directly above (or anywhere before) the macro calls it should affect:

```js
import { t } from "@lingui/core/macro";

// lingui-set context="homepage"
const msg1 = t`Welcome`;
const msg2 = t`Sign up`;
```
...
| ---------- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `context`  | `context="homepage"`        | Sets the [context](/guides/explicit-vs-generated-ids#context) for ID generation                                                                                             |
| `comment`  | `comment="translator note"` | Sets the [comment](#comment) for translators                                                                                                                                |
| `idPrefix` | `idPrefix="module."`        | Prefixes explicit `id` values (does not affect generated IDs). If `macro.idPrefixLeader` is specified in the lingui config, only IDs starting with the leader are prefixed. |
...
```js
// lingui-set context="homepage"
/* lingui-set context="homepage" comment="note" */
// lingui-reset
/* lingui-reset context="new context" */
```

Params use the `key="value"` syntax. Values must be wrapped in double quotes.

#### Merging and Scoping

Directives are **file-scoped**. Each directive merges its values with values from previous directives. This allows you to set a base context at the top of a file and add or override specific values later:
...
const msg2 = t`Cancel`; // context="settings", comment="Updated note"
```

#### Unsetting a Value

Setting a param to an empty string unsets that specific value, without affecting other inherited values:

```js
import { t } from "@lingui/core/macro";

// lingui-set context="settings" comment="note"
const msg1 = t`Save`; // context="settings", comment="note"

// lingui-set context=""
const msg2 = t`Cancel`; // comment="note" (context is removed)
```

#### Resetting Prior Values

The `lingui-reset` directive clears **all** inherited values. You can combine it with new values to immediately set a new context:
...
// lingui-reset
const msg2 = t`Cancel`; // no context & no comment

// lingui-reset context="new section"
const msg3 = t`Delete`; // context="new section"
```

#### Explicit Values Override Directives

Values passed directly to a macro always take precedence over directive values:

```js
import { t } from "@lingui/core/macro";

// lingui-set context="directive ctx"
const msg = t({ message: "Hello", context: "explicit ctx" });
// → context will be "explicit ctx"
```

#### Usage with React (JSX) Macros

The directive works with React macros too:

```jsx
import { Trans } from "@lingui/react/macro";

{/* lingui-set idPrefix="homepage." context="homepage" */}
<Trans>Welcome</Trans>
<Trans id="copyright">Copyright</Trans>
```
...
:::tip Named Tag Placeholders
By default, tags are indexed numerically (`<0></0>`). To provide descriptive placeholder names for translators (e.g. `<link>docs</link>`), configure [`macro.jsxPlaceholderAttribute`](/ref/conf#macrojsxplaceholderattribute) or [`macro.jsxPlaceholderDefaults`](/ref/conf#macrojsxplaceholderdefaults).
You can enforce named placeholders with the Lingui [ESLint Plugin](/ref/eslint-plugin) rule [`no-unnamed-tag-placeholders`](https://github.com/lingui/eslint-plugin/blob/main/docs/rules/no-unnamed-tag-placeholders.md).
:::
