...
### `@lingui` Comment Directive {#lingui-directive}

Instead of passing `context`, `comment`, or `id` prefix to every macro call, you can use the `@lingui` comment directive to set these values for all subsequent macros in the same file (or until the next directive).
...
| `idPrefix` | `idPrefix="module."`           | Prefixes explicit `id` values (does not affect generated IDs) |
| `reset`    | `reset`                        | Clears all inherited directive values (keyword, no value) |
...
```js
// @lingui context="homepage"
/* @lingui context="homepage" comment="note" */
```

Params use the `key="value"` syntax. Values must be wrapped in double quotes. `reset` is a keyword and does not accept a value.
...
// @lingui context="settings" comment="note"
...
The `reset` keyword clears **all** inherited values. You can combine it with new values:
...
{/* @lingui idPrefix="homepage." context="homepage" */}
<Trans>Welcome</Trans>
