...
Create a `lingui.config.js` file in the root of your project (next to `package.json`). Replace `src` with the name of the directory where you have the source files:
...
This configuration will extract messages from source files in the `src` directory and write them to message catalogs in `src/locales` (the English catalog would be in `src/locales/en/messages.po`, for example). See [Configuration](/docs/ref/conf.md) for more options.

The PO format is the default and recommended format for message catalogs. See the [Catalog Formats](/docs/ref/catalog-formats.md) documentation for other available formats.
