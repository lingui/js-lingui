---
title: SolidJS i18n with Lingui
description: Learn how to add internationalization to a SolidJS application using Lingui
---

# Solid Apps Internationalization

In this tutorial, we'll learn how to add internationalization (i18n) to an existing SolidJS application. We'll focus on the most common patterns and best practices for using Lingui in Solid.

:::tip Example
If you're looking for a working solution, check out the [Examples](/examples) page. It contains sample projects with the complete setup using Lingui and Solid.

It includes an example for _Solid with Vite_.
:::

## Installing Lingui

1. Follow the [Installation and Setup](/installation) page for initial setup.
2. Install the [`@lingui/core`](/ref/core) and [`@lingui/solid`](/ref/solid) packages.
...
## Configure Lingui for Solid

We recommend using `defineConfig` from `@lingui/solid/config` to create the configuration. It provides all required options for you.
...
Setup Lingui in `vite.config.ts`:
...
Components need to read information about current language and message catalogs from the [`i18n`](/ref/core#i18n) instance. Lingui uses the [`I18nProvider`](/ref/solid#i18nprovider) to pass the `i18n` instance to your Solid components.

Let's add all required imports and wrap our app inside [`I18nProvider`](/ref/solid#i18nprovider):
...
Using JSX Macros is the easiest way to translate your Solid components. It handles translations of messages, including variables and other Solid components.
...
In general, macros are executed at compile time and serve to transform the source code to make the message writing process easier. Under the hood, all JSX macros are transformed into the runtime component [`Trans`](/ref/solid#trans) (imported from `@lingui/solid`).
...
As you can see, the [`Trans`](/ref/solid#trans) runtime component gets `id` and `message` props with a message in [ICU MessageFormat](/guides/message-format) syntax. We could write it manually, but it's just easier and shorter to write JSX as we're used to and let macros generate the message for us.
...
```jsx title="src/index.js" {4-5,7-12}
import { i18n } from "@lingui/core";
import { I18nProvider } from "@lingui/solid";
import { render } from "solid-js/web";
...
1. Add an [`I18nProvider`](/ref/solid#i18nprovider), this component provides the active language and catalog(s) to other components.
...
:::caution Solid JSX references
Solid JSX values are references. Don't store a JSX element in a variable and use it more than once in a message:

```jsx
const docsLink = <a href="/docs">documentation</a>;

<Trans>
  Read {docsLink} or open {docsLink}.
</Trans>;
```

The second use will move the same DOM node instead of rendering another copy. Use inline JSX or a component so Solid creates a new instance at each position:
...
At first glance, these transformations might seem somewhat unconventional; however, they are straightforward, intuitive, and align well with Solid principles. There is no need to focus on MessageFormat, as the library handles its creation for us. We can write our components as we typically would and simply wrap the text in the [`Trans`](/ref/macro#trans) macro.
...
The `i18n` object can be accessed with the [`useLingui`](/ref/solid#uselingui) hook:

```jsx title="src/Inbox.js" {1,5,6,13}
...
:::tip
MessageFormat allows exact forms, like `=0`. However, JSX props can't start with `=` and can't be numbers either, so we need to write `_N` instead of `=0`.
:::
...
## Internationalization Outside of Solid
...
```jsx title="src/Inbox.js"
import { createMemo } from "solid-js";
import { Trans, Plural, useLingui } from "@lingui/solid/macro";
...
- [`@lingui/solid` Reference](/ref/solid)
- [Examples](/examples)
