...
Let's deal with language switching later... but if you're still curious, take a look at [example](../guides/dynamic-loading-catalogs.md) with Redux and Webpack.
:::
...
All we need to make this heading translatable is wrap it in [`Trans`](../ref/macro.mdx#trans) macro:
...
Under the hood, all JSX macros are transformed into [`Trans`](../ref/react.md#trans) component. Take a look at this short example. This is what we write:
...
See the difference? [`Trans`](../ref/react.md#trans) component receives `id` and `message` props with a message in ICU MessageFormat syntax.
...
We're going to use [CLI](../ref/cli.md) again. Run [`extract`](../ref/cli.md#extract) command to extract messages:
...
After adding the configuration file, let's run [`extract`](../ref/cli.md#extract) command again:
...
That's the message we've wrapped inside [`Trans`](../ref/macro.mdx#trans) macro!
...
If we run [`extract`](../ref/cli.md#extract) command again, we'll see that all Czech messages are translated:
...
That's great! So, how we're going to load it into your app? [Lingui](https://github.com/lingui/js-lingui) introduces concept of compiled message catalogs. Before we load messages into our app, we need to compile them. As you see in the help in command output, we use [`compile`](../ref/cli.md#compile) for that:
...
2.  Wrap messages in [`Trans`](../ref/macro.mdx#trans) macro
...
It isn't necessary to extract/translate messages one by one. This usually happens in batches. When you finalize your work or PR, run [`extract`](../ref/cli.md#extract) to generate latest message catalogs and before building the app for production, run [`compile`](../ref/cli.md#compile).

For more info about CLI, checkout the [CLI reference](../ref/cli.md).
...
Spooky, right? Let's see how this message actually looks in the message catalog. Run [`extract`](../ref/cli.md#extract) command and take a look at the message:
...
It may look a bit _hackish_ at first sight, but these transformations are actually very easy, intuitive and feel very _Reactish_. We don't have to think about the MessageFormat, because it's created by the library. We write our components in the same way as we're used to and simply wrap text in the [`Trans`](../ref/macro.mdx#trans) macro.
...
The message ID is _what all catalogs have in common_ – Lundi and Pondělí represent the same message in different languages. It's also the same as the `id` prop in [`Trans`](../ref/macro.mdx#trans) macro.
...
By default, [Lingui](https://github.com/lingui/js-lingui) generates message ID from the content of [`Trans`](../ref/macro.mdx#trans) macro, which means it uses the source language. However, we can easily override it by setting the `id` prop manually:
...
Cool! Curious how this component is transformed under the hood and how the message looks in MessageFormat syntax? Run [`extract`](../ref/cli.md#extract) command and find out by yourself:
...
The [`Plural`](../ref/macro.mdx#plural-1) is gone and replaced with [`Trans`](../ref/react.md#trans) again! The sole purpose of [`Plural`](../ref/macro.mdx#plural-1) is to generate proper syntax in message.
...
What if we want to use variables or components inside messages? Easy! Either wrap messages in [`Trans`](../ref/macro.mdx#trans) macro or use template literals (suppose we have a variable `name`):
