...
describe('Catalog formats - lingui', function () {
  ...
  it('should add locale', function () {
    ...
    expect(plugin(config).addLocale('en'))
      .toEqual([true, expect.stringMatching('en/messages.json$')])

    // Second run, don't do anything
    expect(plugin(config).addLocale('en'))
      .toEqual([false, expect.stringMatching('en/messages.json$')])
  })

  it('shouldn\'t add invalid locale', function () {
    expect(plugin(createConfig()).addLocale('xyz'))
      .toEqual([false, null])
  })
})
