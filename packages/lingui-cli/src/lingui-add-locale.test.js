...
describe('lingui add-locale', function () {
  it('should fail on unknown locale', function () {
    ...
    mockConsole(console => {
      command(format, ['xyz'])
      expect(console.log).toBeCalledWith(expect.stringContaining('Unknown locale:'))
  ...
  it('should add single locale', function () {
    ...
    mockConsole(console => {
      command(format, ['en'])
      expect(console.log.mock.calls[0]).toEqual([expect.stringContaining('Added locale')])
      expect(console.log.mock.calls[0]).toEqual([expect.stringContaining('en')])
      expect(console.log.mock.calls[2]).toEqual([expect.stringContaining('lingui extract')])
    })
  })
  ...
  it('should add multiple locales', function () {
    ...
    mockConsole(console => {
      ...
      expect(console.log.mock.calls[1]).toEqual([expect.stringContaining('already exists')])
      expect(console.log.mock.calls[1]).toEqual([expect.stringContaining('fr')])

      expect(console.log.mock.calls[3]).toEqual([expect.stringContaining('lingui extract')])
    })
  })
})
