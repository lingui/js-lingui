...
describe("getCatalogForMerge", function() {
  ...
  it("should throw error if mergePath ends with slash", function() {
    ...
    try {
      ...
      expect(e.message).toBe("Remove trailing slash from \"locales/{locale}/bad/path/\". Catalog path isn't a directory, but translation file without extension. For example, catalog path \"locales/{locale}/bad/path\" results in translation file \"locales/en/bad/path.po\".");
    }
  })
  ...
  it("should throw error if {locale} is omitted from mergePath", function() {
    ...
    try {
      ...
      expect(e.message).toBe("Invalid catalog path: {locale} variable is missing");
    }
  })  
})
