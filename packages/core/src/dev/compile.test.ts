...
describe("compile", function() {
  ...
  const prepare = (translation, locale?) =>
    interpolate(compile(translation), locale || "en", englishPlurals)
  ...
  const testVector = [
    ["en", "0.1", "10%", "20%", "3/4/2017", "€0.10", "€1.00"],
    ["fr", "0,1", "10 %", "20 %", "04/03/2017", "0,10 €", "1,00 €"]
  ]
  testVector.forEach(tc => {
    ...
    it("should compile custom format for locale=" + locale, function() {
      const number = prepare("{value, number}", locale)
      expect(number({ value: 0.1 })).toEqual(expectedNumber)

      const percent = prepare("{value, number, percent}", locale)
      ...
      const date = prepare("{value, date}", locale)
      ...
      const currency = prepare("{value, number, currency}", locale)
