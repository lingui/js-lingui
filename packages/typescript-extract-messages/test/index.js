...
import plugin from "@lingui/typescript-extract-messages"
...
function testCase(testName, assertion) {
  const transform = filename => () =>
    plugin(path.join(__dirname, "fixtures", filename), {
      localeDir: LOCALE_DIR
    })
  it(testName, () => assertion(transform))
}

describe("lingui/typescript-extract-messages", function() {
  ...
  testCase(
    ...
    transform => {
      // first run should create all required folders and write messages
      expect(transform("js/integration.js")).not.toThrow()
      // another runs should just write messages
      expect(transform("js/integration.js")).not.toThrow()
  ...
  it("should extract JS translations only once inside React components", function() {
    expect(() =>
      plugin(path.join(__dirname, "fixtures", "jsx", "with-react.js"))
    ).not.toThrow()
